	...
	if (argc == 1) {
		...
		*/
		...
		if (!strcmp("-compile", args[1])) {
			...
			w_file_path.assign(file_name.begin(), file_name.end());

			CHESTNUT_LOG(L"Compilation successfully done : " + w_file_path, log_level::log_success);

			write_file(file_name + ".cir", result);
		}
	}
