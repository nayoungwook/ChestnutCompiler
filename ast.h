...
enum ast_type {
	...
	for_statement_ast = 7,
	bin_expr_ast = 8,
	constant_number_ast = 9,
	identifier_ast = 10,
	constant_string_ast = 11,
	operator_ast = 12,
	paren_ast = 13,
	class_ast = 14,
	new_ast = 15,
...
public:
	...
	int type = base_ast;
	int line_number = 0;
	BaseAST* refers = nullptr;
};

class ConstantNumberAST : public BaseAST {
private:
public:
	std::wstring number_string = L"0";

	ConstantNumberAST(const std::wstring& number) : number_string(number) {
		type = constant_number_ast;
	}
};
...
class ConstantStringAST : public BaseAST {
private:
public:
	std::wstring constant_string; // TODO : maybe do some type check later.
	ConstantStringAST(const std::wstring& constant_string) : constant_string(constant_string) {
		type = constant_string_ast;
	}
};
...
public:
	bool is_static = false;
	std::wstring return_type, function_name, access_modifier = L"default";
	std::vector<VariableDeclarationAST*> parameter;
	std::map<std::wstring, Token*> additional_tokens;
...
public:
	ConstructorDeclarationAST(std::vector<VariableDeclarationAST*>& parameter)
		: FunctionDeclarationAST(L"constructor", parameter, L"void") {
		this->parameter = parameter;
