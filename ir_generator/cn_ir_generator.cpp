...
std::unordered_map<std::wstring, std::unordered_map<std::wstring, MemberFunctionData>> member_function_symbol;
std::unordered_map<std::wstring, std::unordered_map<std::wstring, MemberVariableData>> member_variable_symbol;
...
void declare_builtin_functions() {
	builtin_function_symbol.insert(std::make_pair(L"print", BuiltinFunctionData{ L"print", 0, -1 }));
	builtin_function_symbol.insert(std::make_pair(L"window", BuiltinFunctionData{ L"window",  1, 3 }));
	builtin_function_symbol.insert(std::make_pair(L"load_scene", BuiltinFunctionData{ L"load_scene", 2, 1 }));
	builtin_function_symbol.insert(std::make_pair(L"image", BuiltinFunctionData{ L"image",  3, 4 }));
	builtin_function_symbol.insert(std::make_pair(L"background", BuiltinFunctionData{ L"background", 4, 3 }));
	builtin_function_symbol.insert(std::make_pair(L"random", BuiltinFunctionData{ L"random",  5, 0 }));
	builtin_function_symbol.insert(std::make_pair(L"sin", BuiltinFunctionData{ L"sin",  6, 1 }));
	builtin_function_symbol.insert(std::make_pair(L"cos", BuiltinFunctionData{ L"cos", 7, 1 }));
	builtin_function_symbol.insert(std::make_pair(L"tan", BuiltinFunctionData{ L"tan", 8, 1 }));
	builtin_function_symbol.insert(std::make_pair(L"atan", BuiltinFunctionData{ L"atan", 9, 2 }));
	builtin_function_symbol.insert(std::make_pair(L"abs", BuiltinFunctionData{ L"abs", 10, 1 }));
	builtin_function_symbol.insert(std::make_pair(L"random_range", BuiltinFunctionData{ L"random_range", 11, 2 }));
	builtin_function_symbol.insert(std::make_pair(L"sqrt", BuiltinFunctionData{ L"sqrt", 12, 1 }));
	builtin_function_symbol.insert(std::make_pair(L"text", BuiltinFunctionData{ L"text", 13, 3 }));
	builtin_function_symbol.insert(std::make_pair(L"color", BuiltinFunctionData{ L"color", 14, 3 }));
}

void declare_builtin_variables() {
	VariableData shader_data = { 0, L"shader", false };
	global_variable_symbol.insert(std::make_pair(L"default_shader", shader_data));

	VariableData mouse_data = { 1, L"vector", false };
	global_variable_symbol.insert(std::make_pair(L"mouse", mouse_data));

	VariableData fps_data = { 2, L"number", false };
	global_variable_symbol.insert(std::make_pair(L"fps", fps_data));

}

void create_scope() {
	std::vector<std::unordered_map<std::wstring, VariableData>>* temp = new std::vector<std::unordered_map<std::wstring, VariableData>>;
	temp->push_back({});
	local_variable_symbol.push(temp);
}

void destroy_scope() {
	local_variable_symbol.pop();
}
...
scopes get_scope_of_identifier(std::wstring const& identifier, BaseAST* identifier_ast) {
	std::vector<std::unordered_map<std::wstring, VariableData>>* _local_variable_symbol = local_variable_symbol.top();
	int id = get_local_variable_id(_local_variable_symbol, identifier);
	...
	if (identifier == L"this" || identifier == L"null") {
		...
		if (id == -1) {
			if (member_variable_symbol.find(current_class) != member_variable_symbol.end()
				&& member_variable_symbol[current_class].find(identifier) != member_variable_symbol[current_class].end()) {
				return scope_class;
			}
			else if (member_variable_symbol.find(attr_target_class) != member_variable_symbol.end()
				&& member_variable_symbol[attr_target_class].find(identifier) != member_variable_symbol[attr_target_class].end()) {
				return scope_class;
			}
			else {
				...
				if (parsed_class_data.find(searcher) != parsed_class_data.end()) {
					while (parsed_class_data[searcher]->parent_type != L"") {
						...
						if (member_variable_symbol[searcher].find(identifier) != member_variable_symbol[searcher].end()) {
							return scope_class;
						}
					}
				}
...
scopes get_scope_of_function(std::wstring const& identifier, int line_number) {
	...
	if (global_function_symbol.find(identifier) != global_function_symbol.end()) {
		return scope_global;
	}
	else if (builtin_function_symbol.find(identifier) != builtin_function_symbol.end()) {
		return scope_local;
	}

	if (member_function_symbol[current_class].find(identifier) != member_function_symbol[current_class].end()) {
		return scope_class;
	}
	else {
		std::wstring searcher = current_class;
		while (parsed_class_data[searcher]->parent_type != L"") {
			...
			if (member_function_symbol[searcher].find(identifier) != member_function_symbol[searcher].end()) {
				return scope_class;
			}
		}
	}
...
std::wstring create_identifier_ir(IdentifierAST* identifier_ast) {
	...
	if (identifier_ast->identifier == L"this") {
		...
		if (scope == scope_local) {
			result = L"@LOAD_LOCAL " + std::to_wstring(get_local_variable_id(local_variable_symbol.top(), identifier_ast->identifier))
			...
			result = L"@LOAD_CLASS " + std::to_wstring(get_parent_member_variable_size(searcher) + member_variable_symbol[searcher][identifier_ast->identifier].id)
...
VariableData get_data_of_variable(std::wstring const& identifier, BaseAST* data_ast) {
	...
	if (scope == scope_local) {
		std::vector<std::unordered_map<std::wstring, VariableData>>* area = local_variable_symbol.top();
		for (int i = 0; i < area->size(); i++) {
			if (area->at(i).find(identifier) != area->at(i).end()) {
				std::unordered_map<std::wstring, VariableData>::iterator area_iterator;
				std::unordered_map<std::wstring, VariableData> found_area = area->at(i);
		...
		result.id = member_variable_symbol[attr_target_class][identifier].id;
		result.type = member_variable_symbol[attr_target_class][identifier].type;
		result.is_array = member_variable_symbol[attr_target_class][identifier].is_array;
...
MemberVariableData get_member_variable_data(IdentifierAST* searcher, std::wstring const& attr_target_type, bool is_array) {
	...
	if (attr_target_type == L"vector") {
		...
		while (true) {
			...
			if (member_variable_symbol[class_ast_searcher->name].find(searcher->identifier) != member_variable_symbol[class_ast_searcher->name].end()) {
				member_variable.id
					= member_variable_symbol[class_ast_searcher->name][searcher->identifier].id + get_parent_member_variable_size(class_ast_searcher->name);

				member_variable.name = searcher->identifier;
				member_variable.access_modifier = member_variable_symbol[class_ast_searcher->name][searcher->identifier].access_modifier;
				member_variable.type = member_variable_symbol[class_ast_searcher->name][searcher->identifier].type;
				member_variable.is_array = member_variable_symbol[class_ast_searcher->name][searcher->identifier].is_array;
...
MemberFunctionData get_member_function_data(FunctionCallAST* searcher, std::wstring const& type, bool is_array) {
	...
	if (is_array) {
		...
		int search_loop_count = 0;
		while (true) {
			if (member_function_symbol[class_ast_searcher->name].find(searcher->function_name) != member_function_symbol[class_ast_searcher->name].end()) {
				member_function = member_function_symbol[class_ast_searcher->name][searcher->function_name];
				member_function.id
					= member_function_symbol[class_ast_searcher->name][searcher->function_name].id + get_parent_member_variable_size(class_ast_searcher->name);
...
std::wstring create_assign_ir(BaseAST* ast, int indentation) {
	...
	if (is_single_node) { // store ir for the single node.
		...
		if (scope == scope_local) {
			result += L"@STORE_LOCAL " + std::to_wstring(get_local_variable_id(local_variable_symbol.top(), identifier_ast->identifier))
				+ L" (" + identifier_ast->identifier + L") " + std::to_wstring(identifier_ast->line_number) + L"\n";
		}
		else if (scope == scope_class) {
			std::wstring searcher = current_class;
			while (parsed_class_data[searcher]->parent_type != L"") {
				if (member_variable_symbol[searcher].find(identifier_ast->identifier) != member_variable_symbol[searcher].end()) {
					break;
				}
			...
			result += L"@STORE_CLASS " + std::to_wstring(get_parent_member_variable_size(searcher) + member_variable_symbol[searcher][identifier_ast->identifier].id)
				+ L" (" + identifier_ast->identifier + L") " + std::to_wstring(identifier_ast->line_number);
		}
		else if (scope == scope_global) {
...
const std::wstring create_ir(BaseAST* ast, int indentation) {
	...
	case constructor_declaration_ast: {
		...
		for (int i = 0; i < constructor_declaration_ast->parameters.size(); i++) {
			...
			_local_variable_symbol->at(_local_variable_symbol->size() - 1).insert(
				std::make_pair(constructor_declaration_ast->parameters[i]->names[0],
					VariableData{ (unsigned int)i,constructor_declaration_ast->parameters[i]->var_types[0] }
				));
		}
	...
	case function_declaration_ast: {
		...
		case scope_global:
			...
			break;
		case scope_class:
			id = get_parent_member_function_size(current_class) + member_function_symbol[current_class][function_name].id;
			break;
		}
		...
		for (int i = 0; i < function_declaration_ast->parameters.size(); i++) {
			...
			_local_variable_symbol->at(_local_variable_symbol->size() - 1).insert(
				std::make_pair(function_declaration_ast->parameters[i]->names[0],
					VariableData{ generate_local_variable_id(_local_variable_symbol),	function_declaration_ast->parameters[i]->var_types[0] }
				));
		}
...
#pragma region IfBlock
		...
		local_variable_symbol.top()->push_back({});

		for (int i = 0; i < if_statement_ast->body.size(); i++) {
			append_data(result, create_ir(if_statement_ast->body[i], indentation), 0);
		}

		local_variable_symbol.top()->erase(local_variable_symbol.top()->begin() + local_variable_symbol.top()->size() - 1);
...
#pragma endregion 
		...
		for (int i = 0; i < variable_declaration_ast->var_count; i++) {
			...
			case scope_global:
				...
				global_variable_symbol.insert(std::make_pair(variable_declaration_ast->names[i],
					VariableData{ id, type, variable_declaration_ast->array_var_types[i] != L"" }
				));
				break;
			case scope_local: {
				std::vector<std::unordered_map<std::wstring, VariableData>>* _local_variable_symbol = local_variable_symbol.top();
				...
				id = generate_local_variable_id(_local_variable_symbol);

				_local_variable_symbol->at(_local_variable_symbol->size() - 1).insert(std::make_pair(variable_declaration_ast->names[i],
					VariableData{ id,  type,
			...
			case scope_class:
				...
				id = get_parent_member_variable_size(current_class) + member_variable_symbol[current_class][variable_declaration_ast->names[i]].id;
	...
	case function_call_ast: {
		...
		if (scope == scope_class) { // call function in current class.
			function_id = member_function_symbol[current_class][function_name].id;
			call_type = L"@CALL_CLASS";

			bool able_to_access = member_function_symbol[current_class][function_name].access_modifier != L"private";
		...
		else if (global_function_symbol.find(function_name) != global_function_symbol.end()) { // call function in global area.
		...
		else if (builtin_function_symbol.find(function_name) != builtin_function_symbol.end()) { // call function in builtin area.
